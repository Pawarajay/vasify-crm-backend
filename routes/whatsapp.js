...
//         `
//       SELECT COUNT(*) as total FROM whatsapp_campaigns ${whereClause}
//     `,
//         queryParams,
//       )
...
//       const [campaigns] = await pool.execute("SELECT * FROM whatsapp_campaigns WHERE id = ?", [result.insertId])

//       const campaign = campaigns[0]
//       campaign.target_audience = campaign.target_audience ? JSON.parse(campaign.target_audience) : []
...
//       await pool.execute("UPDATE whatsapp_campaigns SET status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?", [
//         status,
//         id,
//       ])
...
//       // Update message status to delivered (simulated)
//       setTimeout(async () => {
//         await pool.execute('UPDATE whatsapp_messages SET status = "delivered", delivered_at = NOW() WHERE id = ?', [
//           result.insertId,
//         ])
//       }, 1000)
