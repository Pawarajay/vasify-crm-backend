...
//       .isIn(["whatsapp-business-api", "website-development", "ai-agent", "other"])
//       .withMessage("Invalid service"),
//     // NEW: createdBy filter for who created the lead
...
//       // NEW: createdBy filter (admins only)
//       if (createdBy && req.user.role === "admin") {
//         whereClause += " AND l.created = ?";
...
//         FROM leads l
//         LEFT JOIN users u ON l.assigned_to = u.id
//         LEFT JOIN users cu ON l.created = cu.id
//         ${whereClause}
//         ORDER BY l.created_at DESC
//         LIMIT ${Number(limit)} OFFSET ${Number(offset)}
//       `;
...
//   whatsappNumber: "whatsapp_number",
//   service: "service",
//   // NOTE: removed "created" mapping because there is no 'created' column
// };
...
//       .isIn(["whatsapp-business-api", "website-development", "ai-agent", "other"])
//       .withMessage("Invalid service"),
//     // createdBy filter for who created the lead (by user id)
//     query("createdBy").optional().isString().withMessage("createdBy must be a string"),
//   ],
//   async (req, res) => {
//     try {
...
//       // createdBy filter (admins only) – uses created_by column
//       if (createdBy && req.user.role === "admin") {
...
//       // creator of the lead (user id)
//       const createdBy = req.user.userId;

//       const leadId = uuidv4();

//       // FIX: removed 'created' column; use created_by instead
//       await pool.execute(
//         `
//         INSERT INTO leads (
//           id,
...
//testing for new changes (19-12-2025)
...
//   // Allow access if the user owns the lead OR it's unassigned
//   const [rows] = await pool.execute(
...
//   if (lead.assigned_to === req.user.userId || lead.assigned_to == null) {
//     return { ok: true };
//   }
...
// // Convert / delete routes unchanged...
...
//testing for new changes (22-12-2025) - Above code work
...
//       // Ensure we have an authenticated user so created_by is not null
...
//       // creator of the lead (user id from token) – now guaranteed to be present
//       const createdBy = req.user.userId;
...
//testing 2 (22-12-2025) for created_by changes 
...
//Above code work as expected just service & addrss field issue when we convert lead to customer
...
//         // NEW: derive service and address from lead or customerData
//         const service =
//           customerData.service || lead.service || null;
//         const address =
//           customerData.address || lead.address || null; // change 'address' to your actual column name if different
...
//testing
// ... all previous requires, helpers, routes above stay exactly the same ...
